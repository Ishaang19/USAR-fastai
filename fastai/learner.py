...
# Ensure self.path is a Path object to handle string paths 

path = Path(self.path) if not isinstance(self.path, Path) else self.path 

file = join_path_file(file, path/self.model_dir, ext='.pth') 
...
torch.save(self, path/fname, pickle_module=pickle_module, pickle_protocol=pickle_protocol) 
